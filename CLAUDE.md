...
- **launchd must outwait the drain**: the plist's `ExitTimeOut` is the sum of every await after the signal - `VALIDATE_TIMEOUT` for the checkout inspection that runs before the drain task exists, `drain_timeout`, `STOP_GRACE` for the stop, a second `STOP_GRACE` for the pipe drain, `LOG_CLOSE_TIMEOUT`, `PR_BUDGET` for the pull-request sequence a run that exited `0` still owes, and `COMPLETE_BUDGET + RETRY_MAX_DELAY + REQUEST_TIMEOUT` for the completion, which overruns its budget because the budget is checked before the backoff sleep rather than after the attempt. Written by `rxd install` from the `config.toml` it finds. The launchd default is 20 s, which `SIGKILL`s the daemon before the drain and the `complete` are done. Adding an await to the shutdown path means adding its budget to `service::exit_timeout`.
