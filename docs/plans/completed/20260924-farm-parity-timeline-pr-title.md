...
- ➕ the run title is cut to `prdesc::TITLE_LIMIT` characters with `...` (`prdesc::fit_title`, shared with the fallback title) before `open_run`: the farm reads at most 64 KiB of the request, and the `Job` it answers echoes the title with Go's `<`/`>`/`&` escaping into a response this client reads at most 64 KiB of, so a heading near the plan's 1 MiB read limit would either be refused or leave a run minted that the client cannot decode
- ➕ stop waits for the opening `setup` post within its `PROGRESS_POST_TIMEOUT` instead of aborting it, as the Go watcher's `Stop` waits on its `WithoutCancel` post: an aborted post could leave a fast run without `setup`, or, already sent, be processed by the farm after the synchronous `pr`/`failed` and overwrite the terminal phase. `service::exit_timeout` carries a third `PROGRESS_POST_TIMEOUT` for it; on a terminal event `execute` runs that wait alongside `RunningJob::stop`, so a held `setup` post never delays the signals to the process group, as the Go runner stops the container through the job context before the watcher unwinds
- ➕ the `farm-out/<run_id>/` removal runs on the blocking pool under `OUTPUT_REMOVE_TIMEOUT` and is budgeted in `service::exit_timeout`: the child knows the directory from `FARM_PR_FILE` and can fill it with any tree, and the removal happens before the run slot is given back
